...
class ContentMainAdapter(private val mContentItems: List<ContentItem>) : RecyclerView.Adapter<ContentMainAdapter.ContentHolder>() {
    ...
    inner class ContentHolder(itemView: View) : RecyclerView.ViewHolder(itemView), View.OnClickListener {
        ...
        override fun onClick(v: View) {
            ...
            intent.putExtra("CONTENT_DETAIL", mContentItems[adapterPosition].contentId)
...
//
                Glide.with(holder.itemView.context).load(mContentItems[pos].image).into(holder.itemView.activity_image)
                holder.itemView.activity_sender.text = mContentItems[pos].name
                ...
                holder.itemView.activity_num.text = mContentItems[pos].now.toString() + "/" + mContentItems[pos].total.toString()
