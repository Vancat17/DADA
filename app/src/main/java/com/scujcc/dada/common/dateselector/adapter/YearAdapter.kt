package com.scujcc.dada.common.dateselector.adapter


import com.scujcc.dada.common.dateselector.model.DateParams
import com.scujcc.dada.common.dateselector.model.DatePick

import java.util.ArrayList
import java.util.Calendar

/**
 * Created by  范朝波 on 2017/8/29.
 * 微信    ：family997722
 * QQ号    ：1136836811
 */

class YearAdapter(dateParams: DateParams, datePick: DatePick) : com.scujcc.dada.common.dateselector.adapter.DatePickAdapter(dateParams, datePick) {

    override fun getItem(position: Int): String {
        val number = mData[position]
        val value = if (number < 10) "0" + number else "" + number
        return value + "年"
    }

    override fun getCurrentIndex(): Int {
        return mData.indexOf(mDatePick.year)
    }

    override fun refreshValues() {
        val calendar = Calendar.getInstance()
        calendar.time = mDateParams.startDate
        val startYear = calendar.get(Calendar.YEAR)

        calendar.time = mDateParams.endDate
        val endYear = calendar.get(Calendar.YEAR)

        val values = ArrayList<Int>()
        for (i in startYear until endYear) {
            values.add(i)
        }

        setData(values)
    }
}
